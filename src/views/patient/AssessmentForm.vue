      ...
      <a-form :form="form" layout="vertical">
        <template v-if="type === &quot;getSSY&quot;">
          <div v-for="item in listData.questionList" :key="item.id">
            ...
            <template v-if="item.labelList.length > 0">
              <a-form-item v-for="m in item.labelList" :key="m.id" :label="m.text">
              ...
              <div v-for="item in listData.questionList" :key="item.id">
                ...
                >
                  ...
                  <a-radio-group
                    ...
                    v-decorator="['kvList.'+item.name, {initialValue: +item.value, rules: [{required: true, message: '请选择'}]}]"
                  >
                    <a-radio v-for="m in item.choice" :value="m.value" :key="m.id">{{ m.text }}</a-radio>
                  </a-radio-group>
                  <a-checkbox-group
                    v-if="item.type === &quot;MULTIPLE_CHOICE&quot;"
                    v-decorator="['kvList.'+item.name, {rules: [{required: true, message: '请选择'}]}]"
                  >
                    <a-checkbox v-for="m in item.choice" :value="m.value" :key="m.id">{{ m.text }}</a-checkbox>
                  </a-checkbox-group>
                </a-form-item>
              </div>
            </a-collapse-panel>
  ...
  async setData(patientId, type, id) {
    ...
    this.listData.questionList.forEach(function (e, i) {
      if (self.type === 'getAscvd') {
        ...
        if (e.groupName !== '' && self.groupName.indexOf(e.groupName) === -1) {
      ...
      if (self.ssyId === 4 || self.ssyId === 3) {
        for (let m = 0; m < e.choiceList.length; m++) {
          e.choiceList[m].text = self.listData.title.choiceList[m].text
        }
      }
      // 睡眠评估 题目5
      if (self.ssyId === 8) {
        for (let m = 0; m < e.labelList.length; m++) {
          for (let n = 0; n < e.labelList[m].choiceList.length; n++) {
    ...
    validateFields((errors, values) => {
      if (!errors) {
        if (this.type === 'getAscvd') {
          ...
          values.patientId = this.patientId
          for (const key in values.textList) {
          ...
          for (const key in values.choice) {
